...
```{r}
# Test for backward selection only
models = c('backward_selection')
num_sim = 10  # Reduced for testing
num_params = 5
num_params_scene10 = 50  # This won't be used for backward_selection
...
for (model_name in models){
  ...
  # Always set madr_bin = TRUE for backward_selection to skip scenario 10
  model_df = mc_simulation_parallel(model_df_func, 
                                   num_params = num_params, 
                                   num_params_scene10 = num_params_scene10, 
                                   madr_bin = TRUE,  # This will skip scenario 10
                                   total_sim = num_sim)
...
#models = c('rlasso','saturated','glider','adaptive_lasso','madr','backward_selection')
models = c('backward_selection')
num_sim = 1000
num_params = 5
...
for (model_name in models){
  ...
  if(model_name == 'backward_selection') {
    model_df = mc_simulation_parallel(model_df_func, 
                                     num_params = num_params, 
                                     num_params_scene10 = num_params_scene10, 
  ...
  # Save results
  new_file = paste0(model_name, '_df_', num_params, 'var_', num_sim,'.rdata')
  assign(paste0(model_name, '_df_', num_params, 'var_', num_sim,'_df_1000'), model_df)
  save(list = paste0(model_name, '_df_', num_params, 'var_', num_sim,'_df_1000'), file = new_file)
...
# mc_simulation = function(df_func, num_params, num_params_scene10, madr_bin = F, total_sim){
#   df_final = data.frame(scenario = 1:13)
#   if (madr_bin == T){
#     df_final = df_final %>% filter(scenario!=10)
#   }
#   for (sim_num in 1:total_sim){ # number of simulations
#     scenarios_lst = overall_scenario_generator(num_params = num_params, num_params_scene10 = num_params_scene10)
#     ate_est_vec = c()
#     for (scene_num in 1:13){
#       if (scene_num == 10){
...
#       ate_est_vec = c(ate_est_vec, df_loop_lst$ate_est)
#     }
#     df_final[[paste0("sim_", sim_num)]] = ate_est_vec
#   }
#   return(df_final)
# }
```

# Computation of Error (old)
