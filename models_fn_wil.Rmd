...
# Testing SIS LASSO
```{r}
# Test on a single scenario first
test_scenario = overall_scenario_generator(num_params = 5, num_params_scene10 = 10)$scenario_1
test_result = sis_lasso_model(test_scenario)
print(test_result)

# Test with a small simulation
models = c('sis_lasso')
num_sim = 10  # Reduced for testing
num_params = 5
num_params_scene10 = 50  # Not used if skipping scenario 10
...
for (model_name in models){
  ...
  # Always set madr_bin = TRUE for sis_lasso to skip scenario 10
  ...
  # Print a sample of the results to check
  print(head(model_df))
  
  # Save results if everything looks good
  new_file = paste0(model_name, '_df_', num_params, 'var_', num_sim,'.rdata')
  assign(paste0(model_name, '_df_', num_params, 'var_', num_sim,'_df_', num_sim), model_df)
  save(list = paste0(model_name, '_df_', num_params, 'var_', num_sim,'_df_', num_sim), file = new_file)
  
  print(paste0(model_name, " Done!"))
}

# Don't forget to stop the cluster when done
stopCluster(cl)
```
...
for (model_name in models){
  ...
  # Save results
  new_file = paste0(model_name, '_df_', num_params, 'var_', num_sim,'.rdata')
  assign(paste0(model_name, '_df_', num_params, 'var_', num_sim), model_df)
  save(list = paste0(model_name, '_df_', num_params, 'var_', num_sim), file = new_file)
...
overall_var5_simulations_results = list()
for (model_name in models){
  load(paste0(model_name,"_df_5var_1000.rdata"))
  overall_var5_simulations_results[[model_name]] = get(paste0(model_name,"_df_5var_1000"))
}

save(overall_var5_simulations_results, file = "overall_var5_simulations_results.rdata")
```
...
num_sim = 1000
models = c('saturated', 'rlasso', 'glider', 'adaptive_lasso', 'madr', 'backward_selection', 'sis_lasso')
load("overall_var5_simulations_results.rdata")

# Store processed results in a list
mc_ratio_var5_sim1000 = list()

# Process the models
for (model_name in models) {
  
  # Generation of MSE, MC bias, MC SD, MSE (for model itself)
  model_df = overall_var5_simulations_results[[model_name]]
  new_df = results_processor(model_df, model_name)
  new_name = paste0(model_name, "_processed_", num_var, "var_", num_sim)
  assign(new_name, new_df)
  
  # Retrieve saturated model for MC ratio computation
  saturated_mse = get("saturated_processed_5var_1000") %>% select(scenario, saturated_mse_mc)
  saturated_mse_noscene10 = get("saturated_processed_5var_1000") %>% filter(scenario != 10) %>% select(scenario, saturated_mse_mc)
  ...
  if (model_name != "saturated") {
    ...
    mc_ratio_var5_sim1000[[model_name]] = get(new_name) %>%
      left_join(saturated_data, by = "scenario") %>%
      mutate(mse_ratio = saturated_mse_mc / .data[[mse_col_name]])
  }
}

# Save the entire processed_data list (if needed)
save(mc_ratio_var5_sim1000, file = "mc_ratio_var5_sim1000.rdata")
